...
# Load libraries
# install.packages("WhatIf") # Install it for in/extrapolation 
library(MASS)
library(simcf)
library(tile)
...
# Load data
setwd("Documents/UW TA/21WI_CSSS569_CA/lab5/chris_examples/")
file <- "nes00a.csv"
...
nohsTrace <- lineplot(x=xhyp,
                      ...
                      col=col[1],
                      extrapolate=list(data=mdata[,2:ncol(mdata)], # Model data w/o the DV
                                       cfact=nohsScen$x[,2:ncol(nohsScen$x)], # Cf scenarios
...
tile(marriedFDTrace,
     ...
     width=list(null=5,yaxistitle=4,yaxis.labelspace=-0.5),
     output=list(file="marriedFDRR",width=10)
     )
